...
export default function Tickets() {
  ...
  return (
    <div className="relative w-full h-auto">
      <div className="fixed top-0 left-0 w-full h-16 py-2 px-6 flex flex-row items-center justify-between bg-white/10 backdrop-blur-md z-99">
        <div>
          <Link
          to={"/"}
          className="text-xl font-bold text-green-300 tracking-wide"
          >
            Ai Ticket
          </Link>
        </div>
        <div className="flex items-center gap-4 text-base">
          {!token?(
            ...
            {
              user?.role === "admin" ? (
                <Link to="/admin" className="text-base btn btn-ghost btn-sm font-semibold">
                Admin Pannel
              </Link>
              ):(
                <></>
            ...
            onClick={handleLogout}
                className="bg-red-500 text-base btn btn-error text-white btn-soft btn-sm"
              >
                Logout
              </button>
            </>
    ...
    <div className="mt-20 max-w-4xl mx-auto pb-4 md:pb-6 mb-12 animate-fade-in">
      
      {/* --- Header & Form Section --- */}
      <div className="card bg-base-100 shadow-xl border border-base-200 mb-10">
        <div className="card-body">
          <h2 className="card-title text-2xl font-bold mb-2">Create a New Ticket</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <label className="form-control w-full">
              ...
              <textarea
                ...
                className="textarea textarea-bordered focus:textarea-primary w-full h-32 bg-base-200/50"
                required
              ></textarea>
            </label>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="badge badge-neutral">{tickets.length} Total</div>
      </div>
      ...
      {fetchingList ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {tickets.map((ticket) => (
            ...
            >
              <div className="card-body p-5">
                ...
                {/* line-clamp-2 keeps descriptions from making the card massive */}
                ...
                <div className="flex justify-between items-center text-xs text-base-content/50">
                  ...
                  {ticket.status ? (
                    <span className="badge badge-sm badge-outline">{ticket.status}</span>
                  ) : (
          ...
          {tickets.length === 0 && (
            <div className="col-span-full text-center py-12 bg-base-200/50 rounded-xl border border-base-300 border-dashed">
              <p className="text-base-content/60 font-medium">No tickets submitted yet.</p>
              <p className="text-xs text-base-content/40 mt-1">Submit a ticket above to get started.</p>
            </div>
          )}
        </div>
      )}
